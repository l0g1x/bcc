...
export function formatResultsTable(results: BenchmarkResult[]): string {
  const header =
    '| Library | Initial Render | Pan Latency | Zoom Latency | Drag Latency | Hierarchical | Manual Position | Events |'
  const separator =
    '|---------|----------------|-------------|--------------|--------------|--------------|-----------------|--------|'

  const rows = results.map((r) => {
    const renderStatus =
      r.initialRenderMs < 1000
        ? `${r.initialRenderMs.toFixed(0)}ms`
        : `${r.initialRenderMs.toFixed(0)}ms`
    const panStatus =
      r.panLatencyMs < 100 ? `${r.panLatencyMs.toFixed(1)}ms` : `${r.panLatencyMs.toFixed(1)}ms`
    const zoomStatus =
      r.zoomLatencyMs < 100 ? `${r.zoomLatencyMs.toFixed(1)}ms` : `${r.zoomLatencyMs.toFixed(1)}ms`
    const dragStatus =
      r.dragLatencyMs < 100 ? `${r.dragLatencyMs.toFixed(1)}ms` : `${r.dragLatencyMs.toFixed(1)}ms`
