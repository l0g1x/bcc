...
import type { BeadFull, BeadsListResponse, BeadShowResponse, BeadApiError } from '@beads-ide/shared';
...
beads.get('/beads', async (c) => {
  try {
    ...
    if (result.exitCode !== 0) {
      ...
      return c.json(
        errorResponse('Command failed', 'BD_ERROR', errMessage),
        500
      );
    }
    ...
    return c.json(
      errorResponse('Internal server error', 'INTERNAL_ERROR', message),
      500
    );
  }
});
...
beads.get('/beads/:id', async (c) => {
  ...
  try {
    ...
    return c.json(
      errorResponse('Invalid bead ID', 'INVALID_ID', message),
      400
    );
  }
  ...
  try {
    ...
    if (result.exitCode !== 0) {
      ...
      if (
        ...
        return c.json(
          errorResponse('Bead not found', 'NOT_FOUND', `Bead '${id}' not found`),
          404
        );
      }
