 ...
 * Read and return formula TOML content + parsed structure.
 */
formulas.get('/formulas/:name', async (c) => {
 ...
 * Write formula TOML to disk (target for auto-save).
 */
formulas.put('/formulas/:name', async (c) => {
  const name = c.req.param('name')
...
/**
 * POST /api/formulas/:name/cook
 * Invoke bd cook <path> --json, return CookResult.
 */
formulas.post('/formulas/:name/cook', async (c) => {
  const name = c.req.param('name')
...
/**
 * POST /api/formulas/:name/sling
 * Invoke gt sling <formula> <target>, return status.
 */
formulas.post('/formulas/:name/sling', async (c) => {
  const name = c.req.param('name')
