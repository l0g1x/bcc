...
describe('Input Validation', () => {
  describe('validateFormulaName', () => {
    ...
    it('rejects empty or invalid types', () => {
      ...
      expect(() => validateFormulaName(null as unknown as string)).toThrow('Formula name is required');
      expect(() => validateFormulaName(123 as unknown as string)).toThrow('Formula name is required');
    });
...
describe('CLI Execution (Integration)', () => {
  describe('runCli', () => {
    it('rejects invalid binary', async () => {
      await expect(
        runCli('rm' as 'bd', ['-rf', '/'], { cwd: '/tmp' })
      ).rejects.toThrow('Invalid binary');
    });

    it('rejects non-string arguments', async () => {
      await expect(
        runCli('bd', [123 as unknown as string], { cwd: '/tmp' })
  ...
  describe('bdCook', () => {
    it('rejects invalid formula names with shell metacharacters', async () => {
      await expect(
        bdCook('test;rm', undefined, { cwd: '/tmp' })
